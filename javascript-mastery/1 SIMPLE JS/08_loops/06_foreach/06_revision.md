# Foreach Revision

Path: `1 SIMPLE JS\08_loops\06_foreach\06_revision.md`

## Must Remember

- forEach runs a callback for every array item and returns undefined.
- Always predict output before running code.
- Check whether the original value changes or a new value is returned.
- Practice one simple case and one edge case.

## Quick Example

```js
function run(value) {
  return value;
}
console.log(run("Foreach"));
```

## Study Order

1. Read `01_theory.md`.
2. Run `02_snippets.js`.
3. Predict answers in `04_coding-output-practice.js`.
4. Revise this file before interview.

## Interview Sentence

Foreach means: forEach runs a callback for every array item and returns undefined.
